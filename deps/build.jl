...
function compile_gfortran(
        path::AbstractString, basename::AbstractString,
        options::Dict = Dict()
    )
...
function build_ddeabm(path::AbstractString)
    ...
    return link_gfortran(path, ["ddeabm", "slatec"])
...
function build_ddebdf(path::AbstractString)
    ...
    return link_gfortran(path, ["ddebdf", "slatec"])
...
function build_bvpm2(path::AbstractString)
    ...
    return link_gfortran(path, ["bvp_m_proxy", "bvp_m-2", "bvp_la-2"], opt)
end
