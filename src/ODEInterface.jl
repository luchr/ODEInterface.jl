...
"""
function extractTOLs(d::Integer, opt::AbstractOptionsODE)
    rtol = getOption(opt, OPT_RTOL, 1.0e-3)
    atol = getOption(opt, OPT_ATOL, 1.0e-6)
    scalarFlag = false
    ...
    if isscalar(rtol) && isscalar(atol)
        ...
        try
            ...
            throw(
                ArgumentErrorODE(
                    string(
                        "OPT_RTOL and OPT_ATOL are scalars. ",
                        "But I've a problem converting rtol=$rtol,",
                        " and/or atol=$atol to Float64"
                    ), :opt, e
                )
            )
        end
        ...
        try
            ...
            throw(
                ArgumentErrorODE(
                    string(
                        "OPT_RTOL and/or OPT_ATOL is not a real scalar. ",
...
reads options: `OPT_LOGIO`, `OPT_LOGLEVEL`
"""
function solver_start(
        solver_name::AbstractString, rhs,
        t0::Real, T::Real, x0::Vector, opt::AbstractOptionsODE
    )
...
"""
function solver_extract_commonOpt(
