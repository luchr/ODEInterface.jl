...
We have the typical calling stack:
     ...
     odex
         ...
         call_julia_output_fcn(  ... DONE ... )
             output_fcn ( ... DONE ...)
"""
mutable struct OdexInternalCallInfos{
        FInt <: FortranInt,
        RHS_F, OUT_F,
...
"""
function unsafe_odexSoloutCallback_c(
        cbi::CI,
        fint_flag::FInt
    ) where {FInt, CI}
    return @cfunction(
        unsafe_odexSoloutCallback, Cvoid, (
            Ptr{FInt},
...
For the typical calling sequence, see `OdexInternalCallInfos`.
"""
function create_odex_eval_sol_fcn_closure(
        cbi::CI, d::FInt,
...
function odex_impl(
    ...
    try
        ...
        @assert 1.0e-35 < args.WORK[1] < 1.0
        ...
        # H
        OPT = OPT_INITIALSS
        args.H = [convert(Float64, getOption(opt, OPT, 1.0e-4))]
    catch e
        throw(ArgumentErrorODE("Option '$OPT': Not valid", :opt, e))
    end
    ...
    cbi = OdexInternalCallInfos(
        lio, l, rhs, rhs_mode, rhs_lprefix,
