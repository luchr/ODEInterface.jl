...
struct BandedMatrix{T} <: AbstractMatrix{T}
    ...
    function BandedMatrix{T}(
            m::Integer, n::Integer, l::Integer, u::Integer,
...
function BandedMatrix(m::Integer, n::Integer, l::Integer, u::Integer, initval::Number)
    me = max(0, 1 + l + u)
    ne = max(0, n)
    bm = BandedMatrix{typeof(initval)}(
        m, n, l, u,
        Matrix{typeof(initval)}(undef, me, ne)
    )
    fill!(bm, initval)
    return bm
end
...
function setdiagonal!(bm::BandedMatrix, d::Integer, value)
    (-bm.l ≤ d ≤ bm.u) || throw(
        ArgumentErrorODE(
            string(
                "requirement: -l ≤ d ≤ u; d=$d, u=", bm.u, " l=", bm.l
            ), :d
        )
    )
    dIndex = 1 + bm.u - d
    ...
    if length(values) ≠ 1 + bm.u + bm.l
        throw(
            ArgumentErrorODE(
                string(
                    "expected a 1+u+l=",
                    1 + bm.u + bm.l, " array, got length ", length(values)
                ), :values
            )
        )
    end
...
function setindex!(bm::BandedMatrix, value, i::Integer, j::Integer)
    ...
    return bm.entries[1 + bm.u + i - j, j] = value
end
...
function setindex!(bm::BandedMatrix, value, ra::UnitRange, j::Integer)
    ...
    return bm.entries[(ra.start + bm.u + 1 - j):(ra.stop + bm.u + 1 - j), j] = value
end
...
function setindex!(bm::BandedMatrix, value, zra::UnitRange, sra::UnitRange)
    (
        isvalidinband(bm, zra.start, sra.start) &&
            isvalidinband(bm, zra.stop, sra.stop)
    ) || throw(BoundsError(bm, (zra, sra)))
    s = 1
    for nj in sra
        ni = zra.start + bm.u + 1 - nj
        for z in 1:length(zra)
