...
const solvers_bvpsol = (
    bvpsol, bvpsol_i32,
)

const solvers_colnew = (
    colnew, colnew_i32,
)
...
function test_ode1(solver::Function)
    ...
    @assert t == T
    @assert x0 == [1, 2]
    @assert isapprox(x[1], exp(1), rtol = 1.0e-7, atol = 1.0e-7)
    @assert isapprox(x[2], 2 * exp(1), rtol = 1.0e-7, atol = 1.0e-7)
...
function test_ode2(solver::Function)
    ...
    @assert t < T
    if dense_flag
        @assert isapprox(x3, exp(3), rtol = 1.0e-7, atol = 1.0e-7)
    end
    return true
end
...
function test_massode1(solver::Function)
    ...
    (t, x, retcode, stats) = solver(rhs, t0, T, x0, opt)
    @assert 1 == retcode
    @assert isapprox(x[1], x1_exact(T), rtol = 1.0e-7, atol = 1.0e-7)
    @assert isapprox(x[2], x2_exact(T), rtol = 1.0e-7, atol = 1.0e-7)
    return true
end
...
function test_massode2(solver::Function)
    ...
    @assert 1 == retcode
    @assert isapprox(x[1], x1_exact(T), rtol = 1.0e-7, atol = 1.0e-7)
    @assert isapprox(x[3], x3_exact(T), rtol = 1.0e-7, atol = 1.0e-7)
    return true
end
...
function test_massode3(solver::Function)
    ...
    (t, x, retcode, stats) = solver(rhs, t0, T, x0, opt)
    @assert 1 == retcode
    @assert isapprox(x[5], x5_exact(T), rtol = 1.0e-7, atol = 1.0e-7)
    @assert isapprox(x[6], x6_exact(T), rtol = 1.0e-7, atol = 1.0e-7)
    return true
end
...
function test_massode4(solver::Function)
    ...
    (t, x, retcode, stats) = solver(rhs, t0, T, x0, opt)
    @assert 1 == retcode
    @assert isapprox(x[4], x4_exact(T), rtol = 1.0e-7, atol = 1.0e-7)
    return true
end
...
function test_jacode1(solver::Function)
    ...
    t0 = 0; T = 0.2; x0 = [1, 2]
    (t, x, retcode, stats) = solver(
        (t, x) -> [x[1] * (x[2])^2, x[1] * x[2]],
        t0, T, x0, opt
    )
    @assert 1 == retcode
    @assert isapprox(x[1], x1_exact(T), rtol = 1.0e-7, atol = 1.0e-7)
...
function test_jacode4(solver::Function)
    ...
    function myjac(t, x, J1, J2)
        ...
        return setdiagonal!(J2, 0, [2, 2])
    end

    hp = 1 + sqrt(3); hm = 1 - sqrt(3)

    x3_exact = t -> (
        -5 * exp(hm * t) + 3 * sqrt(3) * exp(hm * t) +
            5 * exp(hp * t) + 3 * sqrt(3) * exp(hp * t)
    ) / (2 * sqrt(3))
...
function test_bvp1(solver::Function)
    ...
    @assert code > 0
    @assert t == tNodes
    @assert isapprox(x[1, 1], 5.0, rtol = 1.0e-4, atol = 1.0e-4)
    @assert isapprox(x[2, 1], 1.0, rtol = 1.0e-4, atol = 1.0e-4)
    @assert isapprox(x[1, 2], 0.459387, rtol = 1.0e-4, atol = 1.0e-4)
    @assert isapprox(x[2, 2], 5.07828, rtol = 1.0e-4, atol = 1.0e-4)
...
function test_bvp3(solver::Function)
    ...
    (t, x, code, stats) = solver(f, bc, tNodes, xInit, odesolver, opt)
    @assert code > 0
    @assert isapprox(x[1, 1], 1, rtol = 1.0e-4, atol = 1.0e-4)
    @assert isapprox(x[2, 1], -3.17614, rtol = 1.0e-4, atol = 1.0e-4)
    @assert isapprox(x[1, 2], 0.755201, rtol = 1.0e-4, atol = 1.0e-4)
...
function test_colnew1(solver::Function)
    ...
    opt = OptionsODE(
        ...
        OPT_RTOL => [1.0e-4, 1.0e-4], OPT_MAXSUBINTERVALS => 200
    )

    sol = nothing
    for ε_new in [1.0, 0.5, 0.2, 0.1]
        ε = ε_new
        guess = sol_old !== nothing ? sol_old : initial_guess
        sol, retcode, stats = colnew(
            [a, b], orders, ζ, rhs, Drhs, bc, Dbc,
            guess, opt
        )
        @assert retcode > 0
        sol_old = sol; ε_old = ε
    ...
    @assert isapprox(z₀[1], 0.161671, rtol = 1.0e-3, atol = 1.0e-3)
    @assert isapprox(z₀[2], 1.01863, rtol = 1.0e-3, atol = 1.0e-3)
    return true
end
...
function test_bvpm2_1()
    ...
    function rhs(x, y, p, f)
        ...
        @assert length(p) == 1
        return f[1] = (sin(x)^2 - p[1] * sin(x)^4 / y[1]) / ε
    end
    ...
    (sol[2], retcode[2], stat[2]) = bvpm2_solve(
        guess_obj, rhs, bc, opt,
        Drhs = Drhs
    )
    (sol[3], retcode[3], stat[3]) = bvpm2_solve(
        guess_obj, rhs, bc, opt,
        Dbc = Dbc
    )
    (sol[4], retcode[4], stat[4]) = bvpm2_solve(
        guess_obj, rhs, bc, opt,
        Drhs = Drhs, Dbc = Dbc
    )
    for k in 1:length(retcode)
        @assert retcode[k] == 0
    ...
    for k in 1:(length(retcode) - 1)
        @assert isapprox(z[k], z[k + 1], rtol = 1.0e-4, atol = 1.0e-4)
    end
...
function test_bvpm2_2()
    ...
    guess_obj = Bvpm2()
    bvpm2_init(guess_obj, 2, 1, mylinspace(a, b, 10), [sqrt(0.75), 1.0e-4])
    retcode = Vector{Int64}(undef, 4)
    sol = Vector{ODEInterface.Bvpm2}(undef, 4)
    stat = Vector{Dict}(undef, 4)
    z = Vector{Matrix}(undef, 4)
    ...
    for k in 1:length(retcode)
        for j in 1:length(xx)
            @assert isapprox(z[k][:, j], exact_sol(xx[j]), rtol = 1.0e-4, atol = 1.0e-4)
        end
    end
...
function test_bvpm2_4()
    ...
    for k in 1:length(retcode)
        ...
        @assert isapprox(bvpm2_get_params(sol[k]), [1.0], rtol = 1.0e-4, atol = 1.0e-4)
    end
...
function test_mas_solvers()
    ...
    return @testset "mas-solvers" begin
...
function test_jac_solvers()
    ...
    return @testset "jac-solvers" begin
...
function test_rhsdt_solvers()
    ...
    return @testset "rhs_dt-sol." begin
